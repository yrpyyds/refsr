...
def regiter_attention_editor_diffusers(model, editor: AttentionBase):
    ...
    def register_editor(net, count, place_in_unet, parent_name=""):
        for name, subnet in net.named_children():
            ...
            # 🚫 跳过 ControlNet / adapter / hint 等模块，避免 shape mismatch
            if any(x in full_name.lower() for x in ["controlnet", "hint", "adapter", "temb_proj"]):
                continue

            # ✅ 正常注入 attention 模块
            if subnet.__class__.__name__ in ["Attention"]:#, "CrossAttention"]:
                # try:
                # subnet.forward = ca_forward(subnet, place_in_unet).__get__(subnet, type(subnet))
                from functools import partial
                import types
                forward_fn = partial(ca_forward, place_in_unet=place_in_unet)
                subnet.forward = types.MethodType(forward_fn, subnet)
                count += 1
                # except RuntimeError as e:
                #     if "mat1 and mat2 shapes cannot be multiplied" in str(e):
                #         print(f"[Skip MASACTRL] shape mismatch in {full_name}")
                #         continue
                #     else:
                #         raise e
            elif hasattr(subnet, 'children'):
                count = register_editor(subnet, count, place_in_unet, full_name)
        return count
